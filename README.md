...
Streaming Analysis: Sử dụng cơ chế đọc luồng (PcapReader) thay vì tải toàn bộ file vào RAM, cho phép phân tích các file PCAP dung lượng lớn (GB) mà không gây tràn bộ nhớ.

Đa dạng phát hiện: Nhận diện được từ các cuộc tấn công thô sơ (Ping Flood) đến các kỹ thuật tinh vi (DNS Tunneling, Stealth Scan).

Hỗ trợ IPv4 & IPv6: Tương thích với cả hai thế hệ địa chỉ IP.
...
Bước 1: Tạo dữ liệu giả lập (Tùy chọn)
Nếu chưa có file PCAP thực tế, chạy script này để tạo file mẫu chứa đủ 8 loại tấn công:
